...
# %% Testing values
nPoints = 25
nPoints += 1  # Adding 1 to the number points for making equal steps in the interval [a,b]
...
(x, y, yEr) = intps.valuesForInterpol()  # Generation of sample points
...
def cubicSplineCoeffCalc(x, y):
    ...
    d[0] = 0; d[len(d)-1] = 0
    b[0] = 2*(x[1] - x[0])
    b[len(b)-1] = 2*(x[len(x)-1]-x[len(x)-2])
