...
class dipole():
    ...
    coordinatesPlusCharge: np.ndarray = np.array([0, 0])
    coordinatesMinusCharge: np.ndarray = np.array([1, 1])
    charge: float = 10.0

    def __init__(self, charge: float, coordinates_plus_charge: np.ndarray, coordinates_minus_charge: np.ndarray):
        """
        Simulate the dipole containing 1 negativ and 1 positiv charges.
        ...
        -----------
        charge : float
            Some charge amplitude (not physical).
        coordinates_plus_charge : numpy.ndarray
            2D Coordinates in pixels of a plus charge.
        coordinates_minus_charge : numpy.ndarray
            2D Coordinates in pixels of a minus charge.

        """
        self.coordinatesPlusCharge = coordinates_plus_charge
        self.coordinatesMinusCharge = coordinates_minus_charge
        self.charge = charge
        if (len(self.coordinatesPlusCharge) != 2) or (len(self.coordinatesMinusCharge) != 2):
            raise Exception("Not 2D Coordinates provided")

    def forceFieldCalc(self, x: float, y: float) -> tuple:
        """
        Calculate of hypothetic force field between two charges.
    ...
    def thermoFieldCalc(self, x: float, y: float, decayPower: float, widthOfFlow: float) -> tuple:
        """
        Mimic the force field (thermo flow) created by scanning laser in liquid medium.
    ...
    def createFieldMap(self, size: int, nPoints: int, fieldType: str = "dipole",
                       decayPower: float = 1.5, widthOfFlow: float = 2) -> tuple:
        """
        Show the map of created by a dipole the force field.
    ...
    def updateCoordinates(self, decayPower: float, coordinatesX, coordinatesY, diffusionPower: float,
        ...
        """Update of all coordinates all at one operation."""
