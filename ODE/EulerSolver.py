...
def predcorrEuler(f,h:float,y0:float,xStart:float,xFinish:float) -> tuple:
    """Simple single step (~h) ODE solver with prediction / correction additional step. f - input function (y'(x) = f(x)),\
...
plt.plot(x,yTh,label='theoretical function',linewidth=3); plt.legend(); plt.title("Simple Euler solver")
...
plt.plot(x2,yTh2,label='theoretical function',linewidth=3); plt.legend(); plt.title("Euler solver with prediction / correction")