...
def rungeCutta4(f, h: float, y0: float, xStart: float, xFinish: float) -> tuple:
    """
    Implement simple single step (~h*4) ODE Runge-Cutta solver.

    f - input function (y'(x) = f(x)),
    h - step size, y0 - initial value, xStart - first value for solving an input ODE, xFinish - finishing value for calculations.
