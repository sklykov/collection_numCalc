...
pixel_size = nyq_pixel_size/100.0  # assuming that actual pixel size is little bit smaller than ultimate requirement - Nyquist size
...
if __name__ == "__main__":
    ...
    restored_img_dct = np.abs(idct(cropped_img_dct1)); restored_img_dct *= 255.0/np.max(restored_img_dct)
    ...
    mf = 2; freq_w *= mf; cropped_img_dct = np.zeros(shape=(h, w)); cropped_img_dct[:, 0:freq_w] = img_dct[:, 0:freq_w]
    ...
    # Cropping out starting frequencies
    divider_freq = w // (int(w*0.10)); cropped_img_dct = np.zeros(shape=(h, w)); start_freq = w // divider_freq
    cropped_img_dct[:, start_freq: w] = img_dct[:, start_freq: w]
    plt.figure("Cropped 10% Start Freq-s", figsize=(w_fig, w_fig*ratio))
    plt.imshow(np.log(np.abs(cropped_img_dct) + 1.0), cmap=plt.cm.magma); plt.tight_layout()
    # restoring of the image based on the cropped DCT spectrum, scaling it back to 8 bit initial image
    restored_img_dct = np.abs(idct(cropped_img_dct)); restored_img_dct *= 255.0/np.max(restored_img_dct)
    restored_img_dct = np.round(restored_img_dct, 0).astype(np.uint8)
    plt.figure("Restored 10% Start Freq-s", figsize=(w_fig, w_fig*ratio)); plt.imshow(restored_img_dct, cmap=plt.cm.gray); plt.tight_layout()

    # Some image properties calculation
    l2norm = np.linalg.norm(img_dct); l2norm_cropped = np.linalg.norm(cropped_img_dct1)
    q_m = -np.sum(np.abs(img_dct/l2norm)*np.log2(np.abs(img_dct)/l2norm))
    q_m_cr1 = -np.sum(np.abs(cropped_img_dct1/l2norm_cropped)*np.log2(np.abs(cropped_img_dct1)/l2norm_cropped))
