...
fringe_step_vert = 42  # defines the horizontal fringes density and width
fringe_width = 32  # in pixels
max_fringe_val = (2**16 - 1)//2  # max pixel value on the center
y_fringe_center = fringe_step_vert // 5
# Generate vertical fringes
while y_fringe_center < h:
    # below - modelling the fringes as gaussian blurred fringes
    sigma_fringe = fringe_width / 4
    y = y_fringe_center - fringe_step_vert // 2
    if y < 0:
        y = 0
    elif y >= h:
        y = h-1
    while y < y_fringe_center + (fringe_step_vert // 2):
        if y < h:
    ...
    y_fringe_center += random.choice([fringe_step_vert - 4, fringe_step_vert - 2, fringe_step_vert,
                                      fringe_step_vert + 1, fringe_step_vert + 3, fringe_step_vert + 5])
...
if avalaible_threads//2 - 1 > 2:
    set_n_workers = avalaible_threads//2 - 1
else:
    set_n_workers = 2
for i in range(n_iterations):
    t1 = time.time()
...
fourier_img_half = np.abs(fftshift(rfft2(img_array[0])))
hf_half, wf_half = fourier_img_half.shape
fourier_img_half = fourier_img_half[:, (wf_half-1)//2:wf_half-1:1]
fourier_img_mirrored = np.flip(fourier_img_half, axis=1)
# fourier_img_restored = np.zeros((hf, wf))
# fourier_img_restored = fourier_img_mirrored
fourier_img_restored = fourier_img_half
# hf, wf = fourier_img_half.shape
# fourier_img_draw = fourier_img_half[hf//2 - hf//6:hf//2+hf//6, wf//2 - wf//6: wf//2 + wf//6]
fourier_img_draw = fourier_img_restored
minF = np.min(fourier_img_draw); maxF = np.max(fourier_img_draw)
plt.figure(); plt.imshow(fourier_img_draw, norm=LogNorm(vmin=minF, vmax=maxF)); plt.tight_layout()
