...
def GaussSeidel(testM: TestingMatricies, epsilon: float = 0.01, nDigits: int = 3, nMaxIterations: int = 10):
    """
    Iterative for linear system like Ax = B, there A and B - matricies which should be assigned before
    call this method
    """
    ...
    while i < n:
        if (A[i][i] == 0):
            flag = False
            break  # Finishing checking because already some element == 0
        i += 1
    if (not flag):
...
def StopIterations(delta, nSize: int, epsilon: float):
    ...
    for i in range(nSize):
        ...
        if deltaS > epsilon:
            flag = True
            break  # the single coincidence is enough
    return flag
