...
# %% Similar evalution for another Zernike polynomials (0, 4), (2, 4)
...
Z10 = np.load(Z10_file_path)
diff_Z20_Z10 = np.round(np.absolute(Z20 - Z10), 5)
Z40_file_name = "ZSHwfs40[(0, 4), (2, 4)].npy"; Z40_file_path = os.path.join(calibration_folder, Z40_file_name)
Z40 = np.load(Z40_file_path)
diff_Z40_Z20 = np.round(np.absolute(Z40 - Z20), 5)
Z460_file_name = "ZSHwfs60[(0, 4), (2, 4)].npy"; Z460_file_path = os.path.join(calibration_folder, Z460_file_name)
Z460 = np.load(Z460_file_path)
diff_Z60_Z40 = np.round(np.absolute(Z460 - Z40), 5)
print("******************************************************************************************")
print("Max difference in integral values between sequential # of steps used for their evaluation:")
print("10 -> 20 steps: ", np.max(diff_Z20_Z10))
print("20 -> 40 steps: ", np.max(diff_Z40_Z20))
